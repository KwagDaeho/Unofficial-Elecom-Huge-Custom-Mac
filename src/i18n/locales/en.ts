export const en = {
  tabInfo: "Info",
  tabCustom: "Custom",
  connected: "Connected",
  waiting: "Waiting for HUGE",
  notConnected: "Not connected",
  remappingOn: "Custom input",
  remappingDesc:
    "This app takes ELECOM HUGE trackball input and applies your custom settings.",
  accessibilityTitle: "Accessibility permission required",
  accessibilityStep1: "Click «Grant access».",
  accessibilityStep2:
    "Allow Elecom Huge Custom in the system prompt or Accessibility settings.",
  accessibilityStep3:
    "When permission is granted, the app restarts automatically.",
  grantAccess: "Grant access",
  buttonMapping: "Button mapping",
  clickAction: "Click",
  longPressAction: "Long press",
  longPressEnable: "Long press",
  autoClickEnable: "Auto-click",
  longPressTime: "Long-press duration",
  rawHid: "raw HID",
  pointerScroll: "Pointer & scroll",
  speed: "Pointer speed",
  speedX: "Pointer horizontal speed",
  speedY: "Pointer vertical speed",
  scrollSpeed: "Scroll speed",
  scrollSpeedVertical: "Vertical scroll speed",
  scrollSpeedHorizontal: "Horizontal scroll speed",
  acceleration: "Acceleration",
  invertVertical: "Invert vertical scroll",
  invertHorizontal: "Invert horizontal scroll",
  ballScroll: "Scroll by moving the ball",
  ballScrollToggle: "Toggle key",
  ballScrollHold: "Hold key",
  ballScrollToggleHelp:
    "Press the key to switch ball movement to scroll. Press again to go back.",
  ballScrollHoldHelp: "While the key is held, ball movement becomes scroll.",
  ballScrollInvertVertical: "Invert vertical",
  ballScrollInvertHorizontal: "Invert horizontal",
  ballScrollSpeed: "Scroll speed",
  ballScrollKey: "Set key",
  ballScrollOn: "On",
  customButtonMapping: "Custom button mapping",
  customMappingAdd: "Add",
  customMappingHelp:
    "Combine keyboard modifiers/keys with a HUGE button. e.g. Ctrl + L, Alt + Fn2, F + L",
  customMappingEmpty: "Click Add to create a custom mapping.",
  customMappingTrigger: "Trigger",
  customMappingSetTrigger: "Set trigger",
  customMappingTriggerTitle: "Set trigger",
  customMappingTriggerHint:
    "1) Press a key chord 2) Press a HUGE button 3) Save",
  customMappingTriggerCaptureHint:
    "Hold a modifier (⌘, Ctrl, …) and press a HUGE button (Fn1, L, …). e.g. ⌘ + Fn1. Esc cancels.",
  customMappingTriggerConfirmHint: "Review the preview, then save.",
  customMappingTriggerWaiting: "Press a key chord…",
  customMappingTriggerButtonWaiting: "Press a HUGE button…",
  customMappingTriggerIncomplete:
    "Press a key chord first, then a HUGE button.",
  customMappingTriggerReset: "Reset",
  gestureMapping: "Gesture mapping",
  gestureMappingAdd: "Add",
  gestureMappingHelp:
    "Hold a key, draw a gesture with the ball, then release to trigger an action. Shapes match regardless of size.",
  gestureMappingEmpty: "Click Add to create a gesture mapping.",
  gestureHoldKey: "Hold key",
  gestureHoldKeySet: "Set hold key",
  gestureHoldKeyTitle: "Gesture hold key",
  gestureHoldKeyHint:
    "Press the key or button to hold while drawing the gesture.",
  gestureHoldKeyWaiting: "Waiting for input…",
  gestureHoldRejectedBallScroll: "Cannot match the ball-scroll hold key.",
  gestureShape: "Gesture",
  gestureAction: "Action",
  gestureShapeSet: "Record gesture",
  gestureShapeRecorded: "Edit gesture",
  gestureShapeTitle: "Record gesture",
  gestureShapeHint:
    "Draw straight strokes in 8 directions (→ ↗ ↑ …). Drag on the canvas, or hold L and roll the trackball. (Esc: cancel)",
  gestureShapeRecording: "Drawing…",
  gestureShapeTooShort:
    "Draw a longer stroke with clear direction changes before saving.",
  gestureShapePreviewHint: "Gesture preview",
  gestureRowActions: "Actions",
  gestureShapeReset: "Reset",
  remove: "Remove",
  activatorTitle: "Set a key",
  activatorHint:
    "Press one key, mouse button, or HUGE button. Esc cancels. Left click cannot be used.",
  activatorWaiting: "Waiting…",
  activatorRejectedLeft: "Left click cannot be used.",
  activatorRejectedTilt: "Wheel tilt cannot be used.",
  ballScrollRejectedGesture: "Cannot match a gesture hold key.",
  probe: "Live HID probe",
  probeHelp: "Press buttons to verify report bits.",
  probeEmpty: "No report yet — connect HUGE and click a button.",
  probeIgnored: "(ignored non-mouse report)",
  probeNone: "none",
  launchAtLogin: "Start app automatically",
  startMinimized: "Start app minimized",
  loading: "Loading…",
  failedUi: "Failed to start UI",
  credit: "Theo Kwag",
  creditBy: "by",
  contactLabel: "Contact",
  emailLabel: "E-mail",
  kakaoLabel: "Kakao",
  githubLabel: "GitHub",
  themeLight: "Light mode",
  themeDark: "Dark mode",
  customKeyTitle: "Custom key",
  customKeyHint:
    "Press the chord you want. Modifiers + key are captured together.",
  customKeyWaiting: "Waiting for key…",
  openAppTitle: "Open app",
  openAppHint: "Search and pick an installed application.",
  openAppSearch: "Search apps",
  openAppLoading: "Loading apps…",
  openAppEmpty: "No matches",
  openAppError: "Could not load the app list.",
  macroTitle: "Macro",
  macroHint: "Add keystrokes and delays in order.",
  macroReorderHint: "Drag to reorder",
  addKeystroke: "Add key",
  editKeystroke: "Edit key",
  addDelay: "Add delay",
  editDelay: "Edit delay",
  editStep: "Edit",
  delayMs: "Delay (ms)",
  removeStep: "Remove",
  save: "Save",
  cancel: "Cancel",
  clear: "Clear",
  categories: {
    basic: "Basic",
    mouse: "Mouse",
    browser: "Browser",
    edit: "Edit",
    keys: "Keys",
    system: "Window · System",
    media: "Media",
    launch: "Launch",
    custom: "Custom",
  },
  entries: {
    default: "OS default",
    disabled: "Off",
    mouse_left: "Left click",
    mouse_right: "Right click",
    mouse_middle: "Wheel button",
    mouse_back: "Back (mouse)",
    mouse_forward: "Forward (mouse)",
    double_click: "Double click",
    scroll_left: "Scroll left",
    scroll_right: "Scroll right",
    safari_back: "Safari: Back",
    safari_forward: "Safari: Forward",
    safari_zoom_in: "Safari: Zoom in",
    safari_zoom_out: "Safari: Zoom out",
    copy: "Copy (⌘C)",
    cut: "Cut (⌘X)",
    paste: "Paste (⌘V)",
    undo: "Undo (⌘Z)",
    redo: "Redo (⇧⌘Z)",
    save: "Save (⌘S)",
    return: "Enter/Return",
    esc: "Esc",
    tab: "Tab",
    delete: "Delete",
    mission_control: "Mission Control",
    move_space_left: "Previous desktop",
    move_space_right: "Next desktop",
    app_expose: "App Exposé",
    show_desktop: "Show Desktop",
    launchpad: "Apps",
    spotlight: "Spotlight",
    app_switcher: "App switcher",
    close_window: "Close window",
    volume_up: "Volume up",
    volume_down: "Volume down",
    mute: "Mute",
    previous_track: "Previous track",
    next_track: "Next track",
    play_pause: "Play/Pause",
    open_safari: "Open Safari",
    open_finder: "Finder",
    open_settings: "System Settings",
    open_app_pick: "Open app…",
    custom_key: "Custom",
    macro: "Macro input",
  },
  buttons: {
    left: "L",
    right: "R",
    middle: "Wheel button",
    back: "◀ (Back)",
    forward: "▶ (Forward)",
    fn1: "Fn1",
    fn2: "Fn2",
    fn3: "Fn3",
    wheel_tilt_left: "Scroll tilt (left)",
    wheel_tilt_right: "Scroll tilt (right)",
  },
} as const;
